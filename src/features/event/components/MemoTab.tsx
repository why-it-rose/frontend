...
export default function MemoTab({
  ...
  const handleUpdate = async (memoId: number) => {
    ...
    setSaving(true);
    try {
      await onUpdate?.(memoId, editText.trim());
      setEditingId(null);
    } finally {
      setSaving(false);
    }
  ...
  const groupedByYear = memos.reduce<Record<string, StockMemo[]>>(
    (acc, memo) => {
        ...
        <div className="shrink-0 px-4 py-3 bg-white border-b border-[#f3f4f6]">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[13px] font-semibold text-text-primary">
              {eventInfo.stockName} {sign}
              {Math.abs(eventInfo.changeRate)}%
            </span>
          </div>
        </div>
      )}
          ...
          {memos.length > 0 && (
            <div>
              {Object.entries(groupedByYear).map(([year, list]) => (
                <div key={year}>
                  <p className="pt-1 pb-2 text-[12px] text-[#9ca3af] font-medium">
                    {year}
                  </p>
                  <div className="space-y-2.5 mb-3">
                    {list.map((memo) => (
                      <div
                        key={memo.memoId}
                        className="bg-white rounded-[14px] p-[14px_16px]"
                        style={{ border: "1px solid #eeeeee" }}
                      >
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-xs leading-none text-[#9ca3af]">
                            {formatDate(memo.createdAt)}
                          </span>
                          <div className="relative flex items-center gap-2">
                            {confirmId === memo.memoId && (
                              <div
                                className="absolute right-6 z-30 flex items-center gap-2 px-2.5 py-1.5 rounded-lg shadow-sm whitespace-nowrap"
                                style={{
                                  background: "#fff",
                        ...
                        {editingId === memo.memoId ? (
                          <div>
                            <textarea
                              value={editText}
                              onChange={(e) =>
                                setEditText(e.target.value.slice(0, MAX_LENGTH))
                              }
                            ...
                            <div className="flex justify-between items-center mt-1">
                              ...
                              <div className="flex gap-2">
                                <button
                                  ...
                                  className="text-[12px] text-[#9ca3af] hover:opacity-70"
                                >
                                  취소
                                </button>
                                <button
                          ...
                          <p className="text-[13px] text-[#374151] leading-[1.65] break-all">
                            {memo.content}
                          </p>
                        )}
                      </div>
