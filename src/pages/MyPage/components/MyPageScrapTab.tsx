...
import { MY_PAGE_SCRAPS } from './myPage.mock';
...
type ScrapRow = ScrapItem & { id: string };
...
export default function MyPageScrapTab({
  ...
  const [items, setItems] = useState<ScrapRow[]>(() =>
    MY_PAGE_SCRAPS.map((s, i) => ({
      ...s,
      id: `scrap-${s.code}-${i}-${s.date}`,
    })),
  );
        ...
        {manageMode ? (
          ...
          >
            <SortableContext items={sortableIds} strategy={verticalListSortingStrategy}>
              {items.map((row) => (
                <ScrapSortableEditRow key={row.id} row={row} onRemove={removeItem} />
              ))}
            </SortableContext>
